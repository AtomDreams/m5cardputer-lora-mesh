...
This repository contains an Arduino sketch for the M5Cardputer board, implementing a P2P LoRa mesh network for EU868 ISM (868.1 MHz) to send test messages like "Hello mescore test" hex to public loggers (e.g., anrijs.lv/meshlog). It handles AT commands via UART for RUI4.0.6 RAK3172 module, with robust SF12 params for 5km+ LOS range. Project resolves IDE issues like core dumps and undeclared functions (e.g., updateMenuDisplay). 
...
- Connections: LoRa to Grove Port A (TX/RX/VCC; swap TX/RX if no RX). Antenna: Internal Grove or external 868MHz 3dBi for outdoor LOS.

## Software Requirements
- Arduino IDE 2.x+.
- Libraries: M5Unified (for M5Cardputer display/buttons), HardwareSerial (built-in).
- Tools: esptool.py (for erase_flash), M5Burner (logs), anrijs.lv/meshlog (EU868 monitor).
...
5. Open Serial Monitor (115200 baud) for AT logs.
...
Outdoor LOS test: 100m+ clear south Riga, antenna up — expect +EVT:TXP2P DONE x12 in 2min.
...
- **Compilation Fail**: Include M5Unified; verify ESP32S3 board package.
...
- Logs: Attached from M5Burner (TX success, no RX indoor). 
...
## License
MIT License — see LICENSE file. Credits: Based on M5Stack/RUI4 examples. 
